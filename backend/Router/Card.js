...
Router.delete("/cardDetail/:id", verifyToken, async (req, res) => {
  try {
    ...
    if (result.userid === req.userId) {
      ...
      const dataSumMonney = await cal_sum_order_by_id_card(req?.cardId);
...
Router.post("/addCard", async (req, res) => {
  const { idCustomer, stateCard } = req.body;

  if (!idCustomer || !stateCard) {
    res.status(400).json({
      success: true,
...
Router.post("/addCardItem", verifyToken, async (req, res) => {
  const { idProduct, dongia, quantity } = req.body;

  const check = await check_card_Detail_by_Id(req.cardId, idProduct);
  if (check) {
    ...
    const card = await find_by_name_row_card("id", req.cardId);
    if (card.length <= 0) {
      res.status(400).json({
        success: false,

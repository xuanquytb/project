...
const { find_by_name_row_origin, find_all_Origin, find_by_Id, delete_By_Id, InsertOrigin, UpdateOrigin, Origin } = require("../Models/origin");
    ...
    if (result) {
        try {
            ...
            if (!origins) {
                ...
                return res.status(200).json({ success: true, origins });
            }
        } catch (error) {
            return res.status(500).json({ success: false, message: "Server Error" });
        }

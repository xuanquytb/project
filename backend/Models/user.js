...
const find_by_name_row = function (nameRow, value) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM user WHERE ${nameRow} = '${value}'`, (err, elements) => {
            if (err) {
                return reject(err);
            } else {
...
const delete_By_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`DELETE FROM user WHERE (id = '${id}');`, (error, elements) => {
            if (error) {
                return reject(error);
            } else {
...
const find_by_username = function (username) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM user where username = '${username}'`, (err, elements) => {
            if (err) {
                return reject(err);
            } else {
                resolve(elements[0]);
...
const UpdateUserIdCard = function (userId, cardId) {
    return new Promise((resolve, reject) => {
        dbConn.query(`Update user SET idCard = ${cardId} WHERE (id = '${userId}')`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
                return resolve({ id: element.affectedRows, ...element });
...
const UpdateUserAvata = function (nameAvata, userId) {
    return new Promise((resolve, reject) => {
        dbConn.query(`Update user SET nameAvata = '${nameAvata}' WHERE (id = '${userId}')`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
                return resolve({ id: element.affectedRows, ...element });
...
const find_sum_countUser = () => {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT count(*) as countUser FROM user where idRole = 2`, (error, elements) => {
            if (error) {
                return reject(error);
            }
            return resolve(elements[0]);

...
const find_by_name_row_card = function (nameRow, value) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM card WHERE ${nameRow} = '${value}'`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
...
const find_by_idCard_and_IdCus = function (id, idCard) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM card where idUser = '${id}' and id = ${idCard}`, (err, elements) => {
            if (err) {
                return reject(err);
            } else {
                resolve(elements[0]);
...
const find_Card_and_detailCard = (idCard, idPayment) => {
    return new Promise((resolve, reject) => {
        dbConn.query(
            `SELECT idProduct,quantity FROM carddetail join card on carddetail.idCard = card.id  where card.id = '${idCard}'`,
            (error, elements) => {
                if (error) {
                    return reject(error);
                }
...
const check_card_Detail_by_Id = (idCard, idProduct) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`select * from carddetail where idCard = ${idCard} and idProduct = ${idProduct} and detailstate = 0`, (error, elements) => {
            if (error) {
                return reject(error);
            }
            return resolve(elements[0]);
...
const delete_By_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`DELETE FROM card WHERE (id = '${id}');`, (error, elements) => {
            if (error) {
                return reject(error);
            } else {
...
const delete_Card_Detail_By_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`DELETE FROM carddetail WHERE (id = '${id}')`, (error, elements) => {
            if (error) {
                return reject(error);
            } else {
                return resolve(elements.affectedRows);
...
const findCardByUserId = function (idUser) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM card where userid =  '${idUser}'`, (error, elements) => {
            if (error) {
                return reject(error);
            } else {
                return resolve(elements);
...
const UpdateCard = function (cardUpdate, idUser) {
    return new Promise((resolve, reject) => {
        dbConn.query(`Update card SET idMGG = '${cardUpdate.idMGG}' WHERE (idUser = '${idUser}')`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
...
const GetCard_byUserId = function (idUser) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM card where userid = ${idUser}`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
                return resolve(element[0]);
...
const UpdateCardDetail = function (quantity, sumMoney, id) {
    return new Promise((resolve, reject) => {
        dbConn.query(`Update carddetail SET quantity = '${quantity}',sumMoney = '${sumMoney}' WHERE (id = '${id}');`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
                return resolve({ id: element.affectedRows, ...element });
...
const UpdateIdOrder = function (idcard, idOrder) {
    console.log(idOrder);
    return new Promise((resolve, reject) => {
        dbConn.query(`Update carddetail SET idPayOrder = '${idOrder}' WHERE (idCard = '${idcard}' and detailstate = '0')`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
                return resolve({ id: element.affectedRows, ...element });
...
const Updatedetailstate = function (idcard) {
    console.log(idcard);
    return new Promise((resolve, reject) => {
        dbConn.query(`Update carddetail SET detailstate = '1' WHERE (idCard = '${idcard}')`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
                return resolve({ id: element.affectedRows, ...element });
...
const cal_sum_order_by_id_card = function (idCard) {
    return new Promise((resolve, reject) => {
        dbConn.query(
            `SELECT sum(carddetail.sumMoney) as 'tongthanhtoan' FROM carddetail where idCard = ${idCard} and carddetail.detailstate = 0;`,
            (error, elements) => {
                if (error) {
                    return reject(error);
                } else {

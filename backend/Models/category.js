...
const find_by_name_row_category = function (nameRow, value) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM category WHERE ${nameRow} = '${value}'`, (err, elements) => {
            if (err) {
                return reject(err);
            } else {
...
const find_by_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM category where id = '${id}'`, (error, elements) => {
            if (error) {
                return reject(error);
            }
...
const delete_By_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`CALL deleteCategory('${id}');`, (error, elements) => {
            if (error) {
                return reject(error);
            } else {
...
const UpdateCategoryAvata = function (nameImage, categoryId) {
    return new Promise((resolve, reject) => {
        dbConn.query(`Update category SET image = '${nameImage}' WHERE (id = '${categoryId}')`, (err, element) => {
            if (err) {
                return reject(err);
            } else {
                return resolve({ id: element.affectedRows, ...element });

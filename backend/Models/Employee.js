...
const find_Emp_by_name_row = function (nameRow, value) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM employee WHERE ${nameRow} = '${value}'`, (err, elements) => {
            if (err) {
                return reject(err);
            } else {
...
const find_all_Employee = () => {
    return new Promise((resolve, reject) => {
        dbConn.query("SELECT * FROM employee where idRole = 1", (error, elements) => {
            if (error) {
                return reject(error);
            }
            return resolve(elements);
...
const find_by_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM employee where id = '${id}'`, (error, elements) => {
            if (error) {
                return reject(error);
            }
...
const delete_Emp_By_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`DELETE FROM employee WHERE (id = '${id}');`, (error, elements) => {
            if (error) {
                return reject(error);
            } else {
...
const find_Emp_by_username = function (username) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM employee where username = '${username}'`, (err, elements) => {
            if (err) {
                return reject(err);
            } else {
                resolve(elements[0]);

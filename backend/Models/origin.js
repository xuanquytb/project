...
const find_by_name_row_origin = function (nameRow, value) {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM origin WHERE ${nameRow} = '${value}'`, (err, elements) => {
            if (err) {
                return reject(err);
            } else {
...
const find_by_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`SELECT * FROM origin where id = '${id}'`, (error, elements) => {
            if (error) {
                return reject(error);
            }
...
const delete_By_Id = (id) => {
    return new Promise((resolve, reject) => {
        dbConn.query(`DELETE FROM origin WHERE (id = '${id}');`, (error, elements) => {
            if (error) {
                return reject(error);
            } else {
